...
while True:
    try:
        ...
        #index_name = f"{base_index_name}-{datetime.now().strftime('%Y.%m.%d.%H.%M')}"
        index_name = f"{base_index_name}"
        ...
        print(f"[{doc['@timestamp']}] Log enviado. ID: {resp['_id']} | Resultado: {resp['result']}")
